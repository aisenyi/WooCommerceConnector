...
def link_items(items_list, woocommerce_settings, sys_lang):
	for item_data in items_list:
		...
		existing_item = frappe.db.exists("Item", {"woocommerce_id": item_woo_com_id})

		if not existing_item:
			existing_item = frappe.db.exists("Item", {"woocommerce_product_id": item_woo_com_id})

		woocommerce_sku = item_data.get("sku")
		if not existing_item and woocommerce_sku is not None:
			existing_item = frappe.db.exists("Item", {"item_code": woocommerce_sku})
		...
		if not existing_item:
			...
			item.woocommerce_product_id = item_woo_com_id
...
def create_sales_order(order, woocommerce_settings, customer_name, sys_lang):
	...
	customer = frappe.get_doc("Customer", {"woocommerce_email": order.get("billing").get("email")})
...
def set_items_in_sales_order(new_sales_order, woocommerce_settings, order, sys_lang):
	...
	for item in order.get("line_items"):
		...
		found_item = frappe.db.exists("Item", {"woocommerce_id": cstr(woocomm_item_id)})
		if not found_item:
			found_item = frappe.db.exists("Item", {"woocommerce_product_id": cstr(woocomm_item_id)})

		if not found_item:
			found_item = frappe.db.exists("Item", {"item_code": item.get("sku")})

		if found_item:
			found_item = frappe.get_doc("Item", found_item)
